...
const record = [
  ...
  /*function superbowlWin(record) {
    return record.result === 'y'; // Return true if 'W' is found
  }
  
  const myRecord = record.find(superbowlWin);
  
  if (myRecord) {
    console.log(myRecord.year);
  } else {
    
  }*/

/*if (myRecord) {
  console.log(`Super Bowl win in year ${myRecord.year}`);
} else {
  console.log('No Super Bowl win found.');
}
console.log (superbowlWin(record))
  /*let myrecord=record.find(function(obj){
    let array2=[]
    //return 1st object with result of w
    if (obj.result==='W'){array2.push(obj.year)}else{array2.push('sorry')}
    return array2
  })
  console.log(myrecord)*/
  function superbowlWin(record) {
    const myRecord = record.find((item) => item.result === 'W');
